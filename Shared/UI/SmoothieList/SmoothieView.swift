...
struct SmoothieView: View {
    ...
    var body: some View {
        container
            ...
            .onReceive(accountModel.account) {
                self.account = $0
            }
    }
    ...
    var bottomBar: some View {
        VStack(spacing: 0) {
            Divider()
            Group {
                if let account = account, account.canRedeemFreeSmoothie {
    ...
    func orderSmoothie() {
        _ = orderModel.orderSmoothie(smoothie)
        presentingOrderPlacedSheet = true
    }
    
    func redeemSmoothie() {
        _ = orderModel.redeemSmoothie(smoothie)
        presentingOrderPlacedSheet = true
    }
